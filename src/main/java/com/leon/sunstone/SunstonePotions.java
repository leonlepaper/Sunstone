...
public class SunstonePotions {
	...
	// эффектов, а не во времени.
	private static final int DURATION_1 = 1800;   // 1.5 минуты
	private static final int DURATION_2 = 1800;   // 1.5 минуты

	/**
	 * Третий порядок заметно короче — его ещё пережить надо.
	 *
	 * Число некруглое намеренно. Горение снимает по сердцу в секунду и тянется
	 * ещё пару секунд после конца эффекта, так что 33 секунды стоят игроку 35
	 * здоровья. Регенерация II за свои 22 секунды возвращает 17. От двадцати
	 * остаётся ровно два — одно сердце: под это подогнано достижение
	 * «Закалённый» (см. README). Тронешь число — поедет и его сложность.
	 */
	static final int DURATION_3 = 660;    // 33 секунды
